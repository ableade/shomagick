...
bool createImageFeatureFile(string imageName, string featuresDir) {
  string imageFeatureFile =  featuresDir + imageName + ".xml";
  vector<KeyPoint> keypoints;
  Mat descriptors;
  Detector <SURF> myDetector;
  //Have we already created this image features?
  if (! boost::filesystem::exists(imageFeatureFile)) {
...
int main(int argc, char* argv[]) {
  path featuresDirectory (current_path().string() + "/features/");
  if (! boost::filesystem::exists(featuresDirectory)) {
    boost::filesystem::create_directory(featuresDirectory);
  }
  ...
  vector<vector<string>> candiateMatches(mosaicImages.size());
  ...
  for (auto i = 0; i< v.size(); i++) {
    ...
    candiateMatches[i].push_back(img->fileName);
