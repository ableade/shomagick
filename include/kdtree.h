...
#ifndef KDTREE_INCLUDEDD
#define KDTREE_INCLUDEDD
...
public:
    ...
    /*
     * Given a point v and an integer k, finds the k points in the KDTree
     * nearest to v and returns these points
    */
    std::vector<ElemType> kNearestNeighbours(const Point<N>& key, std::size_t k) const;
...
template <std::size_t N, typename ElemType>
std::vector<ElemType> KDTree<N, ElemType>::kNearestNeighbours(const Point<N>& key, std::size_t k) const {
    BoundedPQueue<ElemType> pQueue(k); // BPQ with maximum size k
    ...
    // Put the k nearest neighbours in a vector
